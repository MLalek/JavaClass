...
public class List01 {
	...
	public static void main(String[] args) {
		...
		1)Arrays are fixed in length but Lists are flexible in length
		 "List are resizable Arrays"
		 2)When you create an array you have to declare the length but for lists no declaration
		...
		*/
		...
		l3.add(2, "Mehmet");
		System.out.println(l3);//[John, Mark, Mehmet, Angie]
		
		//Note: If you use index more than length to add elements you will get 
		//      Run Time Error ==> l3.add(11, "X"); ==> RTE
		...
		System.out.println(l3.size());//8
		
		//How to check if a list if empty or not
		//isEmpty() returns boolean if the list has no element it returns true
		...
		l3.remove("Mark");
		System.out.println(l3);//[John, X, Y, Mehmet, Angie, X, Y]
		System.out.println(l3.remove("X")); // true
		System.out.println(l3);//[John, Y, Mehmet, Angie, X, Y] // it delete the first X
		System.out.println(l3.remove("W"));//false
		System.out.println(l3);//[John, Y, Mehmet, Angie, X, Y]
		
		//Hoew to remove an element in a specific index
		System.out.println(l3.remove(2)); // That method removes Mehmet and returns Mehmet
		System.out.println(l3);
		...
		//RunTE. Do not use set() to add new elements. set() it just for updating
		//l3.set(5,"Z"); ==> RTE
		
		//HOw to check if a specific element exist in a list
		System.out.println(l3.contains("Adam"));// ==> true
		System.out.println(l3.contains("Aaa"));//==>false
		
		//How to sort elments in a list in natural order (Ascending + Alphabetical)
		Collections.sort(l3); // Do not forget how to sort
		System.out.println(l3); // [Adam, John, X, Y, Y]
		
		//How to delete all elements in a list
		l3.clear();
		System.out.println(l3);//[]
		
		//Can we use primitives in list?
